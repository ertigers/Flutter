...
import 'package:boxApp/page/material/material_list.dart';
import 'package:boxApp/page/senior/senior_list.dart';
import 'package:boxApp/provider/college_major_model.dart';
...
class CollegeMajorPage extends StatefulWidget {
  @override
  _CollegeMajorPageState createState() => _CollegeMajorPageState();
}

class _CollegeMajorPageState extends State<CollegeMajorPage>
with AutomaticKeepAliveClientMixin {
  ScrollController _scrollController = ScrollController();
  CollegeMajorModel _model;
  ...
  Widget build(BuildContext context) {
    return ProviderWidget<CollegeMajorModel>(
      model: CollegeMajorModel(),
      onModelInit: (model) {
        // model.loadData();
        _model = model;
      },
  ...
  void _anchorChange({double offsetTop, CollegeMajorModel model}) {
  ...
  // 面包屑导航项
  Widget _buildAnchorItem({AnchorModel anchor, CollegeMajorModel model}) {
    return InkWell(
      onTap: () {
        anchor.active = true;
        model.setAnchorActive(anchor);
        ...
        children: [
          Container(
            ...
            child: ClipRRect(
              ...
              child: Image.asset("assets/images/bg_college_02.png", fit: BoxFit.cover)
  ...
  // 公开课程
  List<Widget> _buildPublicCourse(CollegeMajorModel model) {
  ...
  // 专业资讯
  List<Widget> _buildInfo(CollegeMajorModel model) {
    AnchorModel anchor = model.getAnchor('专业资讯');
    return [
      SliverToBoxAdapter(
        child: Container(
          key: anchor?.key,
          ...
          child: SectionHeader(
            title: "专业·资讯",
            more: () {
              NavigatorManager.push(InfoListSimplePage(title: "专业资讯"));
            }
          ),
        ),
      ),
  ...
  // 学长经验
  List<Widget> _buildExperience(CollegeMajorModel model) {
  ...
  // 专业题库
  List<Widget> _buildExam(CollegeMajorModel model) {
  ...
  // 讲义资料
  List<Widget> _buildMaterial(CollegeMajorModel model) {
      ...
      SliverToBoxAdapter(
        child: Container(
          ...
          child: SectionHeader(
            title: "讲义·资料",
            more: () {
              NavigatorManager.push(MaterialListPage());
            }
          ),
        ),
      ),
  ...
  // 小班课程
  List<Widget> _buildSmallCourse(CollegeMajorModel model) {
  ...
  // 全科定制
  List<Widget> _buildCustomize(CollegeMajorModel model) {
  ...
  // 学长学姐
  List<Widget> _buildSenior(CollegeMajorModel model) {
      ...
      SliverToBoxAdapter(
        child: Container(
          ...
          child: SectionHeader(
            title: "学长·学姐",
            more: () {
              NavigatorManager.push(SeniorListPage());
            }
          ),
        ),
      ),
